...
void QtdSync::loadConfig(bool bClearQtd)
{
    ...
    while (step < 4)
    {
        ...
        if (step < 2)
        {
            ...
            {
                if (step == 1)
                    m_settings.doUpdate = knownQtd.attribute("doIt", "true") == "true" ? true : false;
                if (m_settings.proxyHost == "")
                {
...
#ifdef WIN32
            if (step == 0)
                m_settings.monitor.bDriveLetters = knownQtd.attribute("driveLetters", "true") == "true" ? true : false;
#else
            m_settings.monitor.bDriveLetters = false;
#endif
            if (step == 0)
...
void QtdSync::saveConfig(bool bMonitorOnly)
{
    ...
    if (!m_settings.bSaveComputerConfig)
        return;
...
#define LOG(global, local, x)        \
    if (global)                      \
        gLog += x + QString("\r\n"); \
    if (local)                       \
        log += x + QString("\r\n");  \
    if (pDlg)                        \
        m_pLog->appendPlainText(x);  \
    if (pDlg)                        \
        m_pLog->moveCursor(QTextCursor::End);

#define TRAY(x)      \
    if (m_pTrayIcon) \
    m_pTrayIcon->setToolTip(QString(x))
...
bool QtdSync::doBackup(QStringList files, QMap<QString, QString> config, QMap<QString, QVariant> *pStats)
{
    ...
    if (pDlg)
        m_pGlobalProgress->setValue(m_pGlobalProgress->maximum());
    qApp->processEvents();
    ...
    for (int nFile = 0; nFile < files.count(); nFile++)
    {
        ...
        if (pDlg)
            m_pGlobalProgress->setRange(0, 0);
        ...
        if (pDlg)
            m_pL_BackupSet->setText((bRestore ? tr("Restoring") + " " : "") + m_config.name);
                ...
                if (!bSilent)
                {
                    ...
                    if (backupFolder == "")
                    {
                        if (QMessageBox::question(m_pDlg, tr("Abort Backup"), tr("Abort Backup") + "?", QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes)
                        {
                            ...
                            if (!bRestore)
                                sendBackupReport(m_config, log, true);
                            goto endbackup;
                        }
                        else
                        {
        ...
        if (pDlg)
            m_pGlobalProgress->setRange(0, m_config.folders.count());
        bool failure = false;
        QString curFolder("");
        for (int k = 0; k < m_config.folders.count(); k++)
        {
            ...
            if (pDlg)
                m_pProgressBar->setRange(0, rangeMax);
            ...
            if (pDlg)
                m_pL_Folder->setText(folder);
            curFolder = folder;
            if (curFolder.endsWith("/"))
                curFolder = curFolder.mid(0, curFolder.length() - 1);
            ...
            rsyncArgs << rsyncExperts.split(" ", Qt::SkipEmptyParts);
#else
            ...
            if (!bRestore)
                foreach (QString exFile, m_config.folders.at(k).filesExcluded)
                {
                    if (exFile != "/")
                    {
            ...
            if (pDlg)
                m_pClose->setIcon(QIcon(":/images/cancel.png"));
            if (m_pTrayMenu && m_pAbortBackup)
                m_pAbortBackup->setEnabled(true);
                ...
                if (m_pProcess->waitForReadyRead(nWaitForReady))
                {
                    ...
                    foreach (QString outLine, outLines)
                    {
                        ...
                        if (regExp.indexIn(outLine) != -1 && regExp.numCaptures() > 1)
                        {
                            ...
                            {
                                if (pDlg)
                                    m_pProgressBar->setMaximum(rangeMax = regExp.cap(2).toInt());
                                if (pDlg)
                                    m_pProgressBar->setFormat("%v/%m");
                            }
                            if (pDlg)
                                m_pProgressBar->setValue(nCurCount = (rangeMax - regExp.cap(1).toInt()));
                        }
...
#endif
                            if (pDlg)
                                m_pProgressBarFile->setValue(regExp.cap(1).toInt());
                        }
                        ...
                        if (regExp.indexIn(outLine) != -1 && regExp.numCaptures() > 0)
                        {
                            ...
                            if (pDlg)
                                m_pL_Speed->setText("<b>" + currentSpeedString + "</b>");
                        }
            ...
            if (pDlg)
                m_pGlobalProgress->setValue(k + 1);
        }
        ...
        if (pDlg)
            m_pGlobalProgress->setValue(count);
    }
...
endbackup:
    ...
    if (pDlg)
        m_pProgressBarFile->setValue(m_pProgressBarFile->maximum());
    if (pDlg)
        m_pGlobalProgress->setValue(m_pGlobalProgress->maximum());
    if (pDlg)
        m_pClose->setEnabled(true);
    if (pDlg)
        m_pClose->setIcon(bSuccess ? QIcon(":/images/ok.png") : QIcon(":/images/cancel.png"));
...
bool QtdSync::readQtd(QString content, QString filename)
{
    ...
    for (int i = 0; i < nLst.count(); i++)
    {
        ...
        if (cAtts.contains("name"))
            fold.name = QtdTools::codecSaveString(cAtts.namedItem("name").nodeValue());
        if (fold.name == "")
        {
        ...
        if (version < 2)
        {
            if (cAtts.contains("exclude"))
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
                fold.exclude = QtdTools::codecSaveString(cAtts.namedItem("exclude").nodeValue().split(";", Qt::SkipEmptyParts).join(";"));
#else
                fold.exclude = QtdTools::codecSaveString(cAtts.namedItem("exclude").nodeValue().split(";", QString::SkipEmptyParts).join(";"));
#endif
        ...
        {
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
            fold.exclude = QtdTools::codecSaveString(cNode.firstChildElement("exclude").text().split(";", Qt::SkipEmptyParts).join(";"));
#else
...
QString QtdSync::realDestinationFolder(QString folderName)
{
    ...
    if (m_config.destination.bRemote)
    {
        ...
        if (!destText.endsWith("/") && !m_config.destination.path.startsWith("/"))
            destText += "/";
        destText += m_config.destination.path;

        if (!destText.endsWith("/") && !folderText.startsWith("/"))
            destText += "/";
        destText += folderText;
...
void QtdSync::slot_subfolderStateChanged(int, bool bShowMessage)
{
    ...
    {
        if (bShowMessage)
            QMessageBox::warning(m_pDlg, tr("Subfolder disabled"), tr("You decided NOT to backup into a subfolder!<br><br>The backup of another backup set into the same backup destination will likely <b>corrupt your backup</b>!<br>Please make sure to use your backup destination only for <b>this</b> backup set!"));
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
    QStringList currentExcludes = uiConfig.m_pCB_Exclude->currentText().split(";", Qt::SkipEmptyParts);
#else
...
void QtdSync::slot_configSubfolderStateChanged(int, bool bShowMessage)
{
    ...
    {
        if (bShowMessage)
            QMessageBox::warning(m_pDlg, tr("Subfolder disabled"), tr("You decided NOT to backup into a subfolder!<br><br>This is only allowed if you have only one folder to backup in your backup set.<br><b>If you add another folder this will be enabled again!</b>"));
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
    QStringList excludes = fold.exclude.split(";", Qt::SkipEmptyParts);
#else
...
remotePathsTryAgain:
    ...
    else
    {
        ...
        foreach (QString line, out.split("\n"))
        {
            ...
            {
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
                QString name = line.split("\t", Qt::SkipEmptyParts)[0].trimmed();
#else
...
QString QtdSync::browseForFolder(QString tmpString, bool bIsFile, QString caption)
{
...
#ifndef WIN32
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
    m_tmpString = "/" + m_tmpString.split("/", Qt::SkipEmptyParts).join("/");
#else
    ...
    if (m_tmpString != "" && m_settings.showFolderBindingOptions)
    {
        ...
        if (bSetQtdSync || bSetQtd || true)
        {
            ...
            if (bSetQtdSync)
                locations << "<i>QtdSync</i>";
            if (bSetQtd)
                locations << tr("this backup set file");
            if (bSetQtdFile)
                locations << tr("other backup set files");
...
void QtdSync::slot_destEdited()
{
    if (m_config.destination.bRemote)
    {
        if (m_pLE_Destination->text() != (m_config.destination.bUseSSH ? "ssh://" : "rsync://") + m_config.destination.dest)
        {
            ...
            {
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
                m_config.destination.dest.replace(QRegularExpression("^ssh://"), QString());
#else
            ...
            {
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
                m_config.destination.dest.replace(QRegularExpression("^rsync://"), QString());
#else
                m_config.destination.dest = m_config.destination.dest.replace(QRegExp("^rsync://"), QString(""));
#endif
                slot_remoteDestination();
            }
        }
...
void QtdSync::slot_mailTest()
{
    ...
    else
    {
        ...
        pInfoBox->setStandardButtons(QMessageBox::NoButton);
        pInfoBox->show();
        qApp->processEvents();
...
void QtdSync::slot_elementSelected(QTreeWidgetItem *pItem, int)
{
    ...
    else
    {
        if (!pItem)
            return;
        if (pItem->data(0, Qt::UserRole | 0x01).toInt() == -1)
            return;
...
void QtdSync::slot_doRestore()
{
    ...
    if (id < 0 || id >= m_pTW_Folders->topLevelItemCount())
        return;
    ...
    if (m_currentDlg == eMain)
    {
        ...
        {
            if (i != id)
                config[QString("exclude=%1").arg(m_config.folders.at(i).name)] = "true";
        }
...
{
    if (!pItem || pItem->data(0, Qt::UserRole | 0x02).toBool() || m_nFillingTreeDepth != 0)
        return;
