        ...
        if (pTabBar)
        {
            if (event->type() == QEvent::HoverMove)
            {
                ...
                if (nTabIdx != -1)
                {
                    ...
                    if (pTabWidget)
                        toolTip = pTabWidget->widget(nTabIdx)->toolTip();
                }
            }
        }
...
class QtdWidget : public QWidget
{
    Q_OBJECT
public:
    QtdWidget(QWidget *parent = 0L, Qt::WindowFlags f = Qt::Widget);
    ~QtdWidget();
...
class QtdWin7Dialog : public QDialog
{
    Q_OBJECT
public:
    QtdWin7Dialog(QWidget *parent = 0, Qt::WindowFlags f = Qt::Widget)
        : QDialog(parent, f)
    {
    }
